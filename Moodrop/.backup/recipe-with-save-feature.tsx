...
export default function RecipeScreen() {
  ...
  const handleSave = () => {
    console.log('handleSave called!', { totalPercentage, editMode, recipe });
    
    if (totalPercentage !== 100) {
      console.log('Total percentage is not 100%:', totalPercentage);
      Alert.alert(
        '저장 실패',
        `성분 비율의 합이 ${totalPercentage}%입니다. 100%가 되어야 합니다.`,
        [{ text: '확인' }]
      );
      return;
    }

    // 기본 레시피 이름으로 바로 저장 (나중에 입력 모달로 개선 가능)
    const defaultRecipeName = `${selectedPerfume.perfumeName} 커스텀`;
    
    Alert.alert(
      '레시피 저장',
      `"${defaultRecipeName}"로 저장하시겠습니까?`,
      [
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              // 레시피 데이터를 API 스펙에 맞게 변환
              const composition = Object.entries(recipe).map(([name, weight]) => {
                // 향료 타입 결정 (API 데이터가 있으면 사용, 없으면 추정)
                const perfumeNote = perfumeNotes.find(note => note.name === name);
                let type = 'middle'; // 기본값
                ...
                if (perfumeNote) {
                  ...
                  // 향료 이름으로 타입 추정
                  const topNotes = ['시트러스', '레몬', '오렌지', '베르가못', '자몽', '라임', '오존', '그린', 'mandarin', 'orange'];
                  const baseNotes = ['머스크', '바닐라', '샌달우드', '시더우드', '패촐리', '베티버', '앰버', 'musk'];
                  
                  if (topNotes.some(note => name.toLowerCase().includes(note))) {
                    type = 'top';
                  } else if (baseNotes.some(note => name.toLowerCase().includes(note))) {
                    type = 'base';
                  }
                }
              ...
              console.log('Composition prepared:', composition);

              // 실제 API 호출로 레시피 저장
              const response = await categoryService.createUserRecipe(
                1, // 사용자 ID (숫자)
                defaultRecipeName,
                `${selectedPerfume.perfumeName} 기반 커스텀 레시피`, // 설명
                composition
              );
              ...
              if (response.success) {
                ...
                Alert.alert('저장 실패', response.error || '레시피 저장에 실패했습니다.');
              }
            } catch (error) {
        ...
        {editMode ? (
          <View style={styles.editModeActions}>
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                console.log('Save button pressed!', { totalPercentage, disabled: totalPercentage !== 100 });
                handleSave();
              }}
