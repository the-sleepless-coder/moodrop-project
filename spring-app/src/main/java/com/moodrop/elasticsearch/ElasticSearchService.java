...
public class ElasticSearchService {
    ...
    /**
     * ES에 인덱싱된 향수의 id 최솟값, 최댓값, 총 개수를 반환한다.
     */
    public EsIdRangeDto getIdRange() {
        NativeQuery nativeQuery = NativeQuery.builder()
                .withQuery(Query.of(q -> q.matchAll(m -> m)))
                .withAggregation("min_id", Aggregation.of(a -> a.min(m -> m.field("id"))))
                .withAggregation("max_id", Aggregation.of(a -> a.max(m -> m.field("id"))))
                .withAggregation("total", Aggregation.of(a -> a.valueCount(v -> v.field("id"))))
                .withMaxResults(0)
                .build();
        ...
        long minId = (long) aggs.aggregationsAsMap().get("min_id").aggregation().getAggregate().min().value();
        long maxId = (long) aggs.aggregationsAsMap().get("max_id").aggregation().getAggregate().max().value();
        long total = (long) aggs.aggregationsAsMap().get("total").aggregation().getAggregate().valueCount().value();

        return new EsIdRangeDto(minId, maxId, total);
    }
