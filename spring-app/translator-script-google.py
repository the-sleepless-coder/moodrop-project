...
for _, row in targets.iterrows():
    ...
    try:
        ...
        if len(results) % LOG_STEP==0:
        #if used_chars > next_log_point:
            elapsed = time.time() - start_time
            print(f"[Progress] rows={len(results):,} | used_chars={used_chars:,} | elapsed={elapsed:.1f}s")
        #    next_log_point += LOG_STEP
...
print(f"Translated rows: {len(results)}, Failed: {len(failed)}, Total elapsed: {total_elapsed:.1f}s")
